/*
 Jacaranda2FA local QR renderer.
 QR encoder derived from "QRCode for JavaScript" by Kazuhiko Arase (2009), MIT License.
 No TOTP secret is sent to any external service; QR generation happens in the browser.
*/
(function(global) {
  "use strict";
  var modules = {}, cache = {};
...
module.exports = QRCode;
  ...
  function makeSvg(text, pixelSize) {
    ...
    var size = parseInt(pixelSize, 10);
    if (!size || size < 180) size = 260;
    ...
    return '<svg xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Authenticator setup QR code" ' +
      'viewBox="0 0 ' + total + ' ' + total + '" width="' + size + '" height="' + size + '" ' +
      'shape-rendering="crispEdges" style="max-width:100%;height:auto;background:#fff;padding:8px">' +
      '<rect width="100%" height="100%" fill="#fff"/>' +
      '<path d="' + path.join("") + '" fill="#000"/>' +
      '</svg>';
  }

  global.Jacaranda2FAQRCode = {
    render: function(elementId, text, pixelSize) {
      var el = global.document && global.document.getElementById(elementId);
      if (!el) return;
      el.innerHTML = makeSvg(text, pixelSize);
    },
    makeSvg: makeSvg
  };
})(window);
